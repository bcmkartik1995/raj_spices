...
.review-charts-details {
    max-width: 720px; /* Reduced max width */
    margin: auto;
    padding: 0 15px;
}
...
.zoom:hover {
                                    ...
                                    <div class="details-product-area">
                                        <div class="product-thumb-area">
                                            ...
                                            @if($product->images && count($product->images) > 0)
                                                ...
                                                <div class="thumb-wrapper {{ $index + 1 }} filterd-items {{ $index > 0 ? 'hide' : '' }}">
                                                    <div class="product-thumb zoom" onmousemove="zoom(event)" 
                                                         style="background-image: url({{ env('PRODUCT_IMAGE_URL') . $image }})">
                                                        <img src="{{ env('PRODUCT_IMAGE_URL') . $image }}" alt="{{ $product->name }}">
                                                    </div>
                                                </div>
                                                @endforeach
                                            @else
                                                <div class="thumb-wrapper 1 filterd-items">
                                            ...
                                            <div class="product-thumb-filter-group">
                                                @if($product->images && count($product->images) > 0)
                                                    ...
                                                    <div class="thumb-filter filter-btn {{ $index === 0 ? 'active' : '' }}" 
                                                         data-show=".{{ $index + 1 }}">
                                                        <img src="{{ env('PRODUCT_IMAGE_URL') . $image }}" alt="product-thumb-filter">
                                                    </div>
                                                    @endforeach
                                                @endif
                                            </div>
                                        ...
                                        <div class="contents">
                                            ...
                                            <div class="variation-selector mb--20">
                                                <div class="variation-wrapper p-4 bg-light rounded shadow-sm">
                                                    <div class="mb-3">
                                                        <label class="form-label fw-bold mb-3">Select Variation:</label>
                                                        <div class="variation-buttons d-flex flex-wrap gap-3">
                                                            @foreach($product->variations as $variation)
                                                                <button 
                                                                    wire:click="selectVariation({{ $variation->id }})"
                                                                    class="btn position-relative {{ $selectedVariation == $variation->id ? 'btn-primary shadow-sm' : 'btn-outline-primary' }} {{ $variation->quantity < 1 ? 'opacity-50' : '' }} p-3"
                                                                    {{ $variation->quantity < 1 ? 'disabled' : '' }}
                                                                    aria-label="Select {{ App\Models\ProductVariation::variation($variation->variation_id) }}"
                                                                >
                                                                    <span class="d-block fw-bold mb-1">{{ App\Models\ProductVariation::variation($variation->variation_id) }}</span>
                                                                    <span class="d-block text-{{ $selectedVariation == $variation->id ? 'light' : 'primary' }}">${{ number_format($variation->selling_price, 2) }}</span>
                                            ...
                                            <div class="product-bottom-action mt-3">
                                                @if($selectedVariation)
                                                    @php
                                                        $selectedVar = $product->variations->firstWhere('id', $selectedVariation);
                                                        $existingCartItem = $cartItems->firstWhere('variation_id', $selectedVariation);
                                                    @endphp

                                                    <div class="d-flex flex-column gap-3">
                                                        <div class="stock-info p-2 rounded {{ $selectedVar->quantity > 0 ? 'bg-success-subtle' : 'bg-danger-subtle' }}">
                                                            <div class="d-flex align-items-center gap-2">
                                                                <i class="fas fa-{{ $selectedVar->quantity > 0 ? 'check-circle text-success' : 'times-circle text-danger' }}"></i>
                                                                <span class="fw-medium">{{ $selectedVar->quantity > 0 ? 'In Stock' : 'Out of Stock' }}</span>
                                                                @if($selectedVar->quantity > 0)
                                                                    <span class="text-muted ms-2">({{ $selectedVar->quantity }} units available)</span>
                                                                @endif
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="d-flex flex-column gap-3">
                                                            @if($existingCartItem)
                                                                <div class="quantity-controls d-flex align-items-center border rounded-pill bg-white shadow-sm" wire:key="cart-item-{{ $existingCartItem->id }}">
                                                                    <button class="btn btn-link text-secondary px-3 py-2" 
                                                                            wire:click="decrementCartItem({{ $existingCartItem->id }})"
                                                                            wire:loading.attr="disabled">
                                                                        <i class="fal fa-minus"></i>
                                                                    </button>
                                                                    <span class="px-3 fw-bold">{{ $existingCartItem->qty }}</span>
                                                                    <button class="btn btn-link text-secondary px-3 py-2" 
                                                                            wire:click="incrementCartItem({{ $existingCartItem->id }})"
                                                                            wire:loading.attr="disabled"
                                                                            {{ $existingCartItem->qty >= $selectedVar->quantity ? 'disabled' : '' }}>
                                                                        <i class="fal fa-plus"></i>
                                                                    </button>
                                                                </div>
                                                            @elseif($selectedVar && $selectedVar->quantity > 0)
                                                            ...
                                                            @else
                                                                <button class="rts-btn btn-secondary" disabled>
                                                                    Out of Stock
                                                                </button>
                                                            @endif
                                                        </div>
                                                        ...
                                                        <div class="mt-4 d-flex flex-column gap-3">
                                                            <button wire:click.prevent="addToWishlist({{ $product->id }})" 
                                                                    class="rts-btn {{ $product->isWishlisted ? 'btn-danger' : 'btn-danger' }} d-flex align-items-center gap-2">
                                                                <i class="fa-{{ $product->isWishlisted ? 'solid' : 'light' }} fa-heart"></i>
                                                                Add to Wishlist
                                                            </button>
                                                            <a href="{{ route('website-cart') }}" class="rts-btn btn-primary d-flex align-items-center gap-2">
                                                                <i class="fal fa-shopping-cart"></i>
                                                                Go to cart
                                                            </a>
                                                        </div>
                                                ...
                                                @else
                                                    <div class="d-flex flex-column gap-3">
                                                        <button class="btn btn-secondary rounded-pill px-4 py-2 flex-grow-1" disabled>
                                                            Select a Variation
                                                        </button>
                                                        <button wire:click.prevent="addToWishlist({{ $product->id }})" 
                                                                class="rts-btn {{ $product->isWishlisted ? 'btn-danger' : 'btn-outline-danger' }}">
                                                            <i class="fa-{{ $product->isWishlisted ? 'solid' : 'light' }} fa-heart"></i>
                                                            Add to Wishlist
                                                        </button>
                                                    </div>
                        ...
                        <div class="product-discription-tab-shop mt--50">
                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link " id="home-tab" data-bs-toggle="tab" data-bs-target="#home-tab-pane" type="button" role="tab" aria-controls="home-tab-pane" aria-selected="false">Product Details</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link active" id="profile-tabt" data-bs-toggle="tab" data-bs-target="#profile-tab-panes" type="button" role="tab" aria-controls="profile-tab-panes" aria-selected="true">Customer Reviews ({{ $reviews->count() }})</button>
                                </li>
                            </ul>
                            ...
                            <div class="tab-content" id="myTabContent">
                                ...
                                <div class="tab-pane fade show active" id="profile-tab-panes" role="tabpanel" aria-labelledby="profile-tabt" tabindex="0">
                                    <div class="single-tab-content-shop-details">
                                        <div class="product-details-review-product-style">
                                            <!-- Average Rating Section -->
                                            <div class="average-stars-area-left">
                                                <div class="top-stars-wrapper">
                                                    <h4 class="review">{{ number_format($reviews->avg('rating'), 1) }}</h4>
                                                    <div class="rating-disc">
                                                        <span>Average Rating</span>
                                                        <div class="stars">
                                                            @for ($i = 1; $i <= 5; $i++)
                                                                <i class="fa-solid fa-star {{ $i <= $reviews->avg('rating') ? 'text-warning' : 'text-muted' }}"></i>
                                                            @endfor
                                                ...
                                                <div class="average-stars-area">
                                                    <h4 class="average">{{ round($reviews->count() > 0 ? $reviews->where('rating', '>=', 4)->count() / $reviews->count() * 100 : 0, 1) }}%</h4>
                                                    <span>Recommended ({{ $reviews->where('rating', '>=', 4)->count() }} of {{ $reviews->count() }})</span>
                                                </div>
                                            </div>
                        
                                            <!-- Reviews List -->
                                            <div class="review-charts-details container my-4">
                                                @foreach ($reviews as $review)
                                                    <div class="single-review p-3 shadow-sm rounded mb-3">
                                                        <!-- User Info -->
                                                        ...
                                                        <div style="display: flex;">
                                                            ...
                                                            <div class="stars mb-2" style="width: 80%;">
                                                                @for ($i = 1; $i <= 5; $i++)
                                                                    <i class="fa-solid fa-star {{ $i <= $review->rating ? 'text-warning' : 'text-secondary' }}"></i>
                                                                @endfor
                                                                <p class="text-dark small mb-2">{{ $review->comment }}</p>
                                                            </div>
                                                            
                                                            @if ($review->image)
                                                                <div class="mb-2 text-center">
                                                                    <img src="{{ asset('storage/' . $review->image) }}" alt="Review Image" class="img-fluid rev-image rounded shadow-sm" style="max-width: 80px;">
                                                                </div>
                                                            @endif
                                                        </div>
                                            
                                                        <!-- Percentage Rating -->
                                                        <span class="badge bg-success small">{{ $review->rating * 20 }}% satisfaction</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                            ...
                                            <div class="submit-review-area">
                                                <form wire:submit.prevent="submitReview" id="review-form"  enctype="multipart/form-data" class="submit-review-area">
                                                    ...
                                                    <div class="your-rating">
                                                        ...
                                                        <div class="stars">
                                                            @for ($i = 1; $i <= 5; $i++)
                                                                <i 
                                                                    class="fa-solid fa-star star {{ $i <= $rating ? 'text-warning' : 'text-muted' }}" 
                                                                    data-value="{{ $i }}"
                                                    ...
                                                    <div class="half-input-wrapper">
                                                        ...
                                                        <div class="half-input">
                                                            <input type="text" wire:model.defer="email" placeholder="Your Email *">
                                                            @error('email') <span class="text-danger">{{ $message }}</span> @enderror
                                                        </div>
                                                    </div>
                                                    <textarea wire:model.defer="comment" placeholder="Write Your Review" required></textarea>
                                                    ...
                                                    <button type="submit" class="rts-btn btn-primary">SUBMIT REVIEW</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
    ...
    @if($cartItems->isNotEmpty())
        {{-- <div class="other-cart-items mt-4">
            <h6 class="mb-3">Other Variations in Cart:</h6>
            <div class="cart-items-wrapper">
                @foreach($cartItems as $item)
                    @if($selectedVariation !== $item->variation_id)
                        @php
                            $itemVariation = $product->variations->firstWhere('id', $item->variation_id);
                        @endphp
                        <div class="cart-item mb-2 p-2 bg-light rounded">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <span class="fw-bold">{{ App\Models\ProductVariation::variation($itemVariation->variation_id) }}</span>
                                    <span class="text-muted">(${{ number_format($item->price, 2) }})</span>
                                </div>
                                <div class="quantity-edit d-flex align-items-center gap-2">
                                    <button class="btn btn-sm btn-outline-secondary" 
                                            wire:click="decrementCartItem({{ $item->id }})"
                                            wire:loading.attr="disabled">
                                        <i class="fal fa-minus"></i>
                                    </button>
                                    <span class="px-2 py-1 bg-white rounded">{{ $item->qty }}</span>
                                    <button class="btn btn-sm btn-outline-secondary" 
                                            wire:click="incrementCartItem({{ $item->id }})"
                                            wire:loading.attr="disabled"
                                            {{ $item->qty >= $itemVariation->quantity ? 'disabled' : '' }}>
                                        <i class="fal fa-plus"></i>
                                    </button>
                                </div>
                            </div>
    ...
    <!-- Include SweetAlert JS if not already included -->
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Handle variation change alerts
            window.addEventListener('variation-changed', event => {
                Swal.fire({
                    title: 'Success!',
            ...
            window.addEventListener('show-alert', event => {
                Swal.fire({
                    ...
                    timer: event.detail.type === 'success' ? 2000 : 3000,
                    showConfirmButton: event.detail.type !== 'success'
                });
            });

            // Handle cart updates
            window.addEventListener('cart-updated', event => {
                // You can add additional cart update animations here
            ...
            const mainImage = document.querySelector('.product-thumb img');
            const thumbnails = document.querySelectorAll('.thumb-filter');
            ...
            thumbnails.forEach(thumb => {
                thumb.addEventListener('click', function() {
                    ...
                    mainImage.parentElement.style.backgroundImage = `url(${newImageSrc})`;
                });
            });
    ...
    <script>
     $(document).ready(function() {
        // Prevent Bootstrap tab refresh issue
        $('a[data-bs-toggle="tab"]').on('click', function (e) {
            e.preventDefault();  // Prevent page refresh caused by tab click
            $(this).tab('show');
        });

        // Handle rating click with Livewire
        $('#star-rating .star').on('click', function(e) {
            e.preventDefault();  // Prevent form submission or page refresh
            var ratingValue = $(this).data('value');
            Livewire.emit('setRating', ratingValue);  // Emit event for rating update
        });
    });
        // Highlight stars based on user interaction
        $('#star-rating i').on('mouseover', function() {
        var ratingValue = $(this).data('value');
        $(this).siblings('i').removeClass('text-warning'); // Remove existing highlights
        $(this).prevAll('i').addClass('text-warning'); // Highlight all previous stars
        $(this).addClass('text-warning'); // Highlight the current star
    });

    // Remove highlight on mouse out
    $('#star-rating i').on('mouseout', function() {
        var ratingValue = $(this).data('value');
        $(this).siblings('i').removeClass('text-warning');
        // If the user has selected a rating, highlight the selected stars
        var selectedRating = $('#star-rating .star.selected').data('value');
        if (selectedRating) {
    ...
    $('#star-rating i').on('click', function() {
        var ratingValue = $(this).data('value');
        $(this).addClass('text-warning').siblings('i').removeClass('text-warning'); // Update visual feedback
        $('#star-rating .star').addClass('selected'); // Mark as selected
        @this.set('rating', ratingValue); // Livewire update
    });
    </script>
